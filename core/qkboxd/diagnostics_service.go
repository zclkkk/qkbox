...
func (s *DiagnosticsService) writeDebugBundle(report api.ProductDiagnosticsReport) (string, api.DebugBundleManifest, error) {
	...
	manifest := api.DebugBundleManifest{
		...
		Redaction:      "Profile content, proxy snapshots, provider tokens, URL userinfo, URL paths, query strings, and URL fragments are not included.",
	}
	...
	if err := writeZipText(zw, "README.txt", "qkbox debug bundle\n\nThis bundle is generated by qkbox and excludes profile content, provider tokens, raw platform owner snapshots, and sensitive URL components.\n"); err != nil {
		_ = zw.Close()
		return "", api.DebugBundleManifest{}, err
	}
