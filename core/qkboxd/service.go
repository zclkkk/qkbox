...
func (s *Service) DiagnosticsGetReport(ctx context.Context, _ api.GetDiagnosticsReportRequest) (api.GetDiagnosticsReportReply, *api.StructuredError) {
...
func (s *Service) DiagnosticsCreateDebugBundle(ctx context.Context, _ api.CreateDebugBundleRequest) (api.CreateDebugBundleReply, *api.StructuredError) {
	report, structured := s.buildDiagnosticsReport(ctx)
	if structured != nil {
...
func (s *Service) buildDiagnosticsReport(ctx context.Context) (api.ProductDiagnosticsReport, *api.StructuredError) {
	...
	if reply, structured := s.PlatformGetSystemProxyStatus(ctx, api.GetSystemProxyStatusRequest{}); structured != nil {
...
func diagnosticChecks(report api.ProductDiagnosticsReport) []api.DiagnosticCheck {
	...
	if report.PrivilegedProviderStatus.ExpectedVersion != "" && report.PrivilegedProviderStatus.Version != "" && report.PrivilegedProviderStatus.Version != report.PrivilegedProviderStatus.ExpectedVersion {
		checks = append(checks, api.DiagnosticCheck{
			...
			Reason:   "Privileged provider version does not match qkboxd.",
...
func (s *Service) writeDebugBundle(report api.ProductDiagnosticsReport) (string, api.DebugBundleManifest, error) {
	...
	manifest := api.DebugBundleManifest{
		...
		Redaction:     "Profile content, encrypted blobs, proxy snapshots, provider tokens, URL userinfo, query strings, and URL fragments are not included.",
	}
	...
	if err := writeZipText(zw, "README.txt", "qkbox debug bundle\n\nThis bundle is generated by qkboxd and excludes profile content, encrypted content, provider tokens, and raw platform owner snapshots.\n"); err != nil {
		_ = zw.Close()
		return "", api.DebugBundleManifest{}, err
	}
